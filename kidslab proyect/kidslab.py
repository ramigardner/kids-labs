"""
ASPR Oracle Lab v2 — Modo Solo / CPU
Dos modos: 
  - CPU: rival automático, temporizador.
  - SOLO: sin rival, sin tiempo, con línea guía.
"""
...
from puzzle import LEVELS, Turtle, CELL, C64, check_command_matches_step
...
GUIDE_COL  = (80, 80, 150)   # color para el camino guía en modo SOLO

# ─── Trivia entre niveles ─────────────────────────────────────────────────────
TRIVIA = [
...
# ─── Rana animada (reemplaza la tortuga) ─────────────────────────────────────
class Frog:
    """Rana pixel-art que dibuja el circuito."""
    def __init__(self, role: str, ox: int, oy: int, color, total_steps: int):
        ...
        self.jump_t = 0.0      # animación de salto
        self.jumping = False
        self.t = 0.0
        self.particles = []    # bugs cazados

    def reset(self, ox, oy, color, total_steps):
        self.__init__(self.role, ox, oy, color, total_steps)
    ...
    def draw(self, surf, fonts):
        ...
        # Capuchita hacker / anteojos nerd
        if self.role == "hacker":
...
# ─── CPU rival (solo usado en modo CPU) ────────────────────────────────────────
class CPUPlayer:
    """Rival automático con errores aleatorios y delays humanos."""
    def __init__(self):
        self.step_index = 0
    ...
    def reset_level(self):
        self.step_index = 0
        self.finished_level = False
        self._schedule_next(delay=random.uniform(4, 8))

    def tick(self, pasos, frog: Frog, game) -> bool:
        ...
        if self.step_index >= len(pasos):
            ...
            if not self.has_pet:
                self.has_pet = True
                self.pet = random.choice(
                    PETS["nerd"] if game.my_role=="hacker" else PETS["hacker"]
                ).copy()
                self.pet["hunger"] = 100
            sfx.play("win")
...
# ─── Pantalla de login con selección de modo ─────────────────────────────────
class LoginSolo:
    ...
    def handle(self, ev):
        if ev.type == pygame.KEYDOWN:
            if ev.key == pygame.K_RETURN:
                ...
                sfx.beep(600, 30, "square", 0.05)
            elif ev.key == pygame.K_RIGHT:
                self.mode = "solo"
                self.mode_selected = True
                sfx.beep(800, 30, "square", 0.05)
            else:
                ch = ev.unicode
                if ch and ch.isprintable() and len(self.name) < 10:
                    self.name += ch.upper()
                    sfx.beep(440+len(self.name)*20, 20, "square", 0.05)
    ...
    def draw(self, surf):
        ...
        px(surf, "* ASPR ORACLE LAB * KIDS LAB *",
           W//2, 60, self.fonts["lg"], C64["cyan"], center=True)
        px(surf, "MODO SOLO o VS CPU — SIN RED — SIN SERVIDOR",
           W//2, 88, self.fonts["sm"], DIM_COL, center=True)

        # Panel nombre
        draw_border_box(surf, (440, 140, 400, 120), BORDER_COL, fill=(10,10,40))
        px(surf, "NOMBRE DE JUGADOR:", 460, 164, self.fonts["sm"], DIM_COL)
        bc = CURSOR_COL if int(self.t*2)%2==0 else C64["mid_grey"]
        draw_border_box(surf, (460, 184, 360, 28), bc, fill=(0,0,20))
        disp = (self.name + ("|" if int(self.t*2)%2==0 else "")) or " "
        px(surf, disp, 468, 190, self.fonts["sm"], C64["white"])

        # Selección de modo
        px(surf, "SELECCIONA MODO (← →)", 640, 290, self.fonts["sm"], C64["yellow"], center=True)
        mode_x = 440
        cpu_rect = (mode_x, 310, 160, 40)
        solo_rect = (mode_x+200, 310, 160, 40)
        if self.mode == "cpu":
            draw_border_box(surf, cpu_rect, C64["lt_green"], fill=(0,40,0), thickness=3)
            draw_border_box(surf, solo_rect, DIM_COL, fill=(20,20,30), thickness=1)
        else:
            draw_border_box(surf, cpu_rect, DIM_COL, fill=(20,20,30), thickness=1)
            draw_border_box(surf, solo_rect, C64["lt_green"], fill=(0,40,0), thickness=3)
        px(surf, "VS CPU", mode_x+80, 325, self.fonts["md"], C64["white"], center=True)
        px(surf, "SOLO (GUÍA)", mode_x+200+80, 325, self.fonts["md"], C64["white"], center=True)

        # Descripción del modo
        if self.mode == "cpu":
            desc = "Rival automático con errores, temporizador y mascotas."
        else:
            desc = "Sin CPU, sin tiempo. Sigue la línea punteada."
        px(surf, desc, 640, 370, self.fonts["xs"], DIM_COL, center=True)

        # Enter
        pulse = abs(math.sin(self.t*3))
        bc2 = tuple(int(80+pulse*175) for _ in range(3))
        draw_border_box(surf, (460, 420, 360, 36), bc2, fill=(8,20,8))
        px(surf, "ENTER — JUGAR", 640, 430, self.fonts["sm"], C64["lt_green"], center=True)

        if self.error:
            px(surf, self.error, W//2, 480, self.fonts["sm"], ERR_COL, center=True)

        # Ranitas decorativas
        px(surf, "🐸", 360, 400, self.fonts["emoji"])
        px(surf, "🐸", 870, 400, self.fonts["emoji"])


# ─── Pantalla de reveal de rol ────────────────────────────────────────────────
class RoleReveal:
...
class GameOverScreen:
    ...
    def draw(self, surf):
        ...
        px(surf, "ASPR ORACLE LAB · KIDSLAB.IO",
           W//2, H//2+60, self.fonts["xs"], DIM_COL, center=True)

        if int(self.t*2)%2==0:
            px(surf, "CERRÁ LA VENTANA PARA SALIR",
               W//2, H//2+100, self.fonts["xs"], C64["mid_grey"], center=True)
...
# ─── Pantalla principal de juego (ahora con soporte para modo SOLO/CPU) ──────
class GameSolo:
    def __init__(self, fonts, name, level_index=0, mode="cpu"):
        self.fonts  = fonts
        self.name   = name
        self.mode   = mode           # "cpu" o "solo"
        self.t      = 0.0
        self.blink  = 0.0
        ...
        # Rana CPU (solo en modo cpu, se dibuja pero no se mueve si no hay cpu)
        if self.mode == "cpu":
        ...
        # Para el camino guía en modo SOLO
        self.guide_points = []   # lista de (x_celda, y_celda) esperados
    ...
    def _compute_guide_path(self):
        """Calcula la trayectoria esperada para el modo SOLO (línea guía)"""
        if self.mode != "solo":
            return
        # Simular la rana en posición inicial
        cx, cy = 5.0, 5.0
        angle = 90.0
        points = [(cx, cy)]
        for paso in self.pasos:
            ...
            if cmd == "fd":
                ...
                # interpolar puntos intermedios? solo el destino final
                cx, cy = nx, ny
                points.append((cx, cy))
            elif cmd == "bk":
                rad = math.radians(angle)
        ...
        if ev.type == pygame.KEYDOWN:
            ...
            if ev.key == pygame.K_f and ctrl_pressed:
                if self.has_pet:
                    ...
                    self.add_log("🍖 ¡Mascota alimentada!", C64["lt_green"])
            elif ev.key == pygame.K_BACKSPACE:
    ...
    def _submit(self):
        ...
        # Convertir a mayúsculas para comparar (pero mostramos como se escribió)
        inp_upper = inp.upper()
        ok, msg = check_command_matches_step(inp_upper, step, lv["comandos_validos"])
        ...
        if ok:
            ...
            if self.step_index >= len(self.pasos):
                ...
                if not self.has_pet:
                    self.has_pet = True
                    role_pets = PETS.get(self.my_role, PETS["nerd"])
                    self.pet_data = random.choice(role_pets).copy()
                    self.pet_data["hunger"] = 100
                    self.pet_hunger = 100
                    sfx.play("pet")
                    self.add_log(f"🐸 {self.pet_data['name']} desbloqueado!", C64["yellow"])
    ...
    def update(self, dt) -> str:
        """Retorna 'trivia', 'gameover' o '' según estado."""
        self.t      += dt
        self.blink  += dt
    ...
    def _draw_guide(self, surf):
        """Dibuja el camino guía (puntos interconectados) en modo SOLO"""
        ...
        # También marcar el punto actual (siguiente paso)
    ...
    def _draw_anim(self, surf):
        ...
        # Dibujar camino guía si es modo SOLO
        ...
        if self.mode == "cpu" and self.cpu_frog:
            ...
            # En modo SOLO, mostrar un mensaje amigable
        ...
        if self.has_pet and self.pet_data:
            ...
            pet_surf = self.fonts["emoji"].render(
                self.pet_data.get("emoji","⭐"), True, C64["white"])
            surf.blit(pet_surf,(pet_x-pet_surf.get_width()//2, pet_y+b))
            px(surf, self.pet_data.get("name",""),
    ...
    def _draw_console(self, surf):
        ...
        # ── PASO ACTUAL — GRANDE Y BIEN VISIBLE ─────────────────────────
        if self.step_index < len(self.pasos):
        ...
        # Lista compacta de todos los pasos
        paso_y=ry+124
...
class App:
    def __init__(self):
        pygame.init()
        pygame.display.set_caption("ASPR Oracle Lab — Kids Lab 🐸")
        self.screen = pygame.display.set_mode((W, H))
        self.clock  = pygame.time.Clock()
        self.fonts  = load_fonts()
        threading.Thread(target=sfx.init, daemon=True).start()
    ...
    def _launch_game(self, idx):
        ...
        if hasattr(self, '_saved_pet') and self._saved_pet:
        ...
        if self.state == "reveal" and self.reveal:
            ...
            if result == "trivia":
                ...
                if self.game_mode == "cpu":
                    ...
                    # modo solo: siempre gana el jugador
                    self.player_wins += 1
