...
PET_TYPES = [
    ...
    {"name": "Ladrón",    "emoji": "🐶", "poder": "Olfato de bugs"},
...
def normalize_input(text: str) -> str:
    """Convierte FD(50) o FD 50 o fd50 al formato estándar FD 50."""
    import re
    text = text.strip().upper()
    # Formato con paréntesis: FD(50) → FD 50
    m = re.match(r'^([A-Z]+)\(([^)]+)\)$', text)
    if m:
        return f"{m.group(1)} {m.group(2)}"
    # Sin espacio: FD50 → FD 50
...
class LoginSolo:
    ...
    def draw(self, surf):
        ...
        # Sombra
        sh = self.fonts["title"].render("🧪 KIDSLAB — ORACLE KIDS", False, (0,0,60))
        surf.blit(sh, (W//2 - sh.get_width()//2 + 2, 22))
        ...
        instrucciones = [
            ...
            ("KIDSLAB","→ libro de secretos"),
            ("GRACE","→ comando secreto 🦟"),
            ("",""),
            ("CON MASCOTA", "usás paréntesis:"),
            ("ADELANTE(4)", "→ mismo efecto"),
        ]
        iy = 110
        ...
        if self.mode == "cpu":
            desc = "Competís contra la máquina"
        else:
            desc = "Sin rival — seguís la guía"
        ...
        # ── Panel derecho: mascotas disponibles ───────────────────────────
...
class TriviaScreen:
    ...
    def draw(self, surf):
        ...
        # Mostrar mascota si la tiene
        if hasattr(self, "pet_data") and self.pet_data:
            pet_surf = self.fonts["emoji"].render(self.pet_data["emoji"], True, C64["white"])
            surf.blit(pet_surf, (W//2+40, 412+bounce))
            px(surf, self.pet_data["name"], W//2+70, 444,
...
class GameSolo:
    ...
    def _submit(self):
        ...
        # Normalizar: acepta FD 50, FD(50), fd50, etc.
        inp = normalize_input(inp_raw)

        lv   = LEVELS[self.level_index]
        step = self.pasos[self.step_index]

        # Siempre validar contra el formato estándar (sin paréntesis)
