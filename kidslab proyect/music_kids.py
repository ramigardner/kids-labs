...
def generate_kick():
    ...
    return _make_sound_from_array(arr_int16)
...
class BeatLab:
    ...
    def load_project_data(self, filename):
        """Carga solo datos (grid, bpm) sin modificar el estado actual."""
        path = os.path.join(self.projects_dir, filename)
        if not os.path.exists(path):
            return None, None
        with open(path, 'r') as f:
    ...
    def reset_grid(self):
        ...
        self.grid = [[False]*GRID_COLS for _ in range(GRID_ROWS)]
        self.set_status("Grilla limpiada")
    ...
    def draw_control_buttons(self):
        ...
        buttons = [
            ...
            ("🗑 RESET", False, (120, 40, 40))   # Botón reset
        ]
        self.button_rects = []
