...
# ─── Mascotas (nuevo sistema) ─────────────────────────────────────────────────
PET_TYPES = [
    {"name": "Saltarina", "emoji": "🐸"},
    {"name": "Tortu",     "emoji": "🐢"},
    {"name": "Misifú",    "emoji": "🐱"},
    {"name": "Ladrón",    "emoji": "🐶"},
]
...
class GameOverScreen:
    ...
    def handle_event(self, ev):
        if ev.type == pygame.KEYDOWN:
            if ev.key == pygame.K_r or ev.key == pygame.K_R:
...
# ─── Pantalla principal de juego (modo CPU/SOLO + mascota con paréntesis) ────
class GameSolo:
    def __init__(self, fonts, name, level_index=0, mode="cpu"):
        ...
        self.step_index   = 0
        self.has_pet      = False
        self.pet_data     = None      # será uno de PET_TYPES
        self.pet_hunger   = 100
        self.pet_anim_t   = 0.0
    ...
    def _submit(self):
        ...
        # Si tenemos mascota, el comando esperado debe llevar paréntesis
        if self.has_pet:
            # Transformar el comando original "FD 50" a "FD(50)"
            cmd_parts = step["cmd"].split()
            if len(cmd_parts) == 2:
                expected = f"{cmd_parts[0]}({cmd_parts[1]})"
            else:
                expected = step["cmd"]  # fallback
        else:
            expected = step["cmd"]

        # Validación simple: comparar con el comando esperado (puede tener espacio o paréntesis)
        # Usamos la función original de puzzle.py, pero adaptamos el comando esperado
        # Como check_command_matches_step espera el step original, creamos un step temporal
        temp_step = step.copy()
        temp_step["cmd"] = expected
        ok, msg = check_command_matches_step(inp_upper, temp_step, lv["comandos_validos"])
        ...
        if ok:
            ...
            if self.step_index >= len(self.pasos):
                ...
                # Dar mascota solo en modo CPU, solo si aún no tiene y acaba de ganar
                if self.mode == "cpu" and not self.has_pet:
                    self.has_pet = True
                    self.pet_data = random.choice(PET_TYPES).copy()
                    self.pet_hunger = 100
                    sfx.play("pet")
                    self.add_log(f"🎉 ¡Has ganado a {self.pet_data['name']} {self.pet_data['emoji']} como mascota!", C64["yellow"])
                    self.add_log("⚠️ Ahora los comandos llevan PARÉNTESIS: EJ: FD(50)", C64["yellow"])
    ...
    def _draw_console(self, surf):
        ...
        if self.step_index < len(self.pasos):
            ...
            # Mostrar el comando tal como debe escribirse (con o sin paréntesis)
            if self.has_pet:
        ...
        for i,paso in enumerate(self.pasos):
            ...
            if i < self.step_index:
                ...
                col=(50,50,90); prefix=" "
            # Mostrar también aquí con paréntesis si tiene mascota
            if self.has_pet:
